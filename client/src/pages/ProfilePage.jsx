...
function normalizeLinkedIn(value) {
  ...
  if (!trimmed) return ''
  if (!trimmed.includes('.') && !trimmed.includes('/')) return `https://www.linkedin.com/in/${trimmed.replace(/^@/, '')}`
...
export default function ProfilePage() {
  ...
  return <main className="profile-page career-profile-page">
    ...
    <form className="profile-form" onSubmit={save}>
      ...
      <Section number="07" title="Portfolio links" help="Help employers see more of your professional work."><div className="field-grid"><label className="linkedin-field"><span>LinkedIn profile</span><div className="url-input"><span aria-hidden="true">in</span><input type="url" inputMode="url" placeholder="linkedin.com/in/your-name" value={profile.portfolio.linkedin || ''} onChange={(e) => setField('portfolio', { ...profile.portfolio, linkedin: e.target.value })} onBlur={(e) => setField('portfolio', { ...profile.portfolio, linkedin: normalizeLinkedIn(e.target.value) })} pattern="https?://(www\.)?linkedin\.com/in/[^\s]+/?" title="Enter a LinkedIn profile URL, for example https://www.linkedin.com/in/your-name" /></div><small>Paste your profile link or enter your LinkedIn username.</small></label>{['github', 'website'].map((name) => <Field key={name} label={name === 'website' ? 'Portfolio / website' : 'GitHub'} type="url" placeholder={name === 'github' ? 'https://github.com/username' : 'https://yourwebsite.com'} value={profile.portfolio[name] || ''} onChange={(e) => setField('portfolio', { ...profile.portfolio, [name]: e.target.value })} />)}</div></Section>
